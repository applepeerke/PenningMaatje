...
class MessageBox(Base):
    ...
    def message_box(
            self, message, cont_text=False, title=None, severity=MessageSeverity.Info, auto_close_duration=None,
            key=None) -> bool:
        """
        ...
        if auto_close_duration is not None and auto_close_duration == 0:
            ...
            value = self._CM.get_config_item(CF_AUTO_CLOSE_TIME_S)
        ...
        if self._session.unit_test:
            return self._session.unit_test_auto_continue if question else False
    
        # Question
        if question:
            answer = sg.PopupYesNo(message, title=title, button_color=button_color, font=self._CM.get_font(),
                                   background_color=POPUP_COLOR_BACKGROUND, text_color=TEXT_COLOR, keep_on_top=True,
                                   line_width=width, icon=self._session.get_icon())
        ...
        # Not a question
            ...
            if key and severity == MessageSeverity.Info:
                ...
                sg.Popup(message, title=title, button_color=button_color, font=self._CM.get_font(),
                         background_color=POPUP_COLOR_BACKGROUND, text_color=TEXT_COLOR, keep_on_top=True,
                         auto_close=auto_close, auto_close_duration=auto_close_duration, line_width=width,
